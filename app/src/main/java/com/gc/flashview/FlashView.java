...
import com.zwj.basedemo.util.DensityUtils;
...
public class FlashView extends FrameLayout {
	...
	private int doctNorId = R.drawable.dot_hover;
	private int doctFouctId = R.drawable.dot_focus;
	...
	private void initUI(Context context) {
		...
		// imageLoaderTools =
		// ImageLoaderTools.getInstance(context.getApplicationContext());
	...
	public void setImageUris(List<String> imageuris) {
		...
		{
			// TODO 处理
			imageUris.add("drawable://" + R.drawable.pictures_no);
		} else {
			if (imageuris.size() == 2) {
				isTwo = true;
		...
		LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
				DensityUtils.dp2px(context, 8), DensityUtils.dp2px(context, 8));
		lp.setMargins(DensityUtils.dp2px(context, 4), 0,
				DensityUtils.dp2px(context, 4), 0);
		for (int i = 0; i < imageUris.size(); i++) {
			ImageView imageView = new ImageView(getContext());
//			new ImageBuilder(context , imageView, imageUris.get(i), ImageBuilder.LoadMode.URL).setScaleType(ScaleType.FIT_XY).build();

			imageViewsList.add(imageView);
			ImageView viewDot = new ImageView(getContext());
			if (i == 0) {
				viewDot.setBackgroundResource(doctFouctId);
			} else {
				viewDot.setBackgroundResource(doctNorId);
			}
	...
	private void setImageBackground(int selectItems) {
		for (int i = 0; i < dotViewsList.size(); i++) {
			if (i == selectItems % dotViewsList.size()) {
				dotViewsList.get(i).setBackgroundResource(doctFouctId);
			} else {
				dotViewsList.get(i).setBackgroundResource(doctNorId);
			}
		}
	}
	...
	private class MyPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(View container, int position) {
			...
			if(view.getTag() == null){
				new ImageBuilder(context , view, imageUris.get(position), ImageBuilder.LoadMode.URL).setScaleType(ImageView.ScaleType.FIT_XY).build();
			}else {
				LogUtils.i("FlashView", "tag not null");
			}
	...
	private class MyPageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int pos) {
			...
			if (mOnPageSelectedListener != null) {
				mOnPageSelectedListener.onPageSelected(pos);
			}
		}
	}
...
//			setPageTransformer(true, new RotateTransformer());
	...
	public void setDotLayoutVisibility(boolean visibility) {
		if(visibility) {
